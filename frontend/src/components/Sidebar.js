...
export const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <aside className={`fixed lg:relative top-0 left-0 h-screen w-[260px] z-50 lg:z-auto transform transition-transform duration-200 shrink-0 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} flex flex-col gradient-sidebar`}>
      ...
      <div className="p-5 border-b border-white/10">
        <div className="flex items-center gap-3">
          {schoolLogo ? (
            <img src={schoolLogo} alt="" className="w-9 h-9 rounded-xl object-cover ring-2 ring-white/20" />
          ) : (
            <div className="w-9 h-9 rounded-xl gradient-card-blue flex items-center justify-center">
              <GraduationCap className="w-5 h-5 text-white" />
            </div>
          )}
          <div className="flex-1 min-w-0">
            <h1 className="font-bold text-white text-sm truncate">{schoolData?.name || 'School'}</h1>
              ...
              <div className="space-y-0.5">
                {group.items.map((item) => (
                  <NavLink key={item.path} to={item.path} onClick={handleNavClick}
                    className={({ isActive }) => `flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] transition-all ${isActive ? 'bg-white/15 text-white font-semibold shadow-sm' : 'text-white/60 hover:bg-white/5 hover:text-white/90'}`}>
      ...
      <div className="p-4 border-t border-white/10">
        <NavLink to="/app/profile" onClick={handleNavClick} className="flex items-center gap-3 px-3 py-2.5 mb-1 rounded-xl hover:bg-white/5 transition-colors">
          <div className="w-8 h-8 gradient-card-purple rounded-full flex items-center justify-center flex-shrink-0">
        ...
        <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2.5 text-xs text-white/50 hover:bg-white/5 rounded-xl transition-colors">

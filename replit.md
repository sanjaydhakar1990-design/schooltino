...
- **AI Paper Drawing Images Enhanced**: Drawing/Art subjects now use substring matching for detection (handles "Drawing (Art)", "Art & Craft" etc). Auto-generates SVG reference images for ALL questions inline in question paper with bordered "Reference Image" boxes and "Drawing Space" areas for print. Backend `is_drawing` flag triggers child-friendly colorful SVGs (500x400). Print CSS supports color rendering.
- **Global Language Toggle in Header**: Hindi/English toggle button added to Layout.js header bar (shows "हि"/"EN"). Accessible from any page without going to Settings.
- **Fee Management Full Bilingual**: All text in FeeManagementPage.js respects `isHindi` flag from LanguageContext - headings, tabs, buttons, table headers, dialog labels, status badges, toast messages (including all error toasts). Fee type names use `name_hi`/`name` based on language.

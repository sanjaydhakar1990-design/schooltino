from fastapi import FastAPI, APIRouter, HTTPException, Depends, status, UploadFile, File, Form, Body, Header
...
@api_router.get("/storage/export-all")
async def export_all_data(current_user: dict = Depends(get_current_user)):
    """REAL export: dumps ALL collections to JSON for migration/backup.
       Director/admin only. Download this JSON before migrating away from emergent."""
    if current_user["role"] not in ["director", "admin"]:
        raise HTTPException(status_code=403, detail="Only director/admin can export")

    from fastapi.responses import JSONResponse

    critical_collections = [
        "schools", "users", "staff", "students", "parents", "classes",
        "subjects", "subject_allocations", "timetable", "timetables",
        "attendance", "exams", "exam_schedules", "exam_results", "exam_entry_logs",
        "marks", "homework", "homework_submissions",
        "fees", "fee_plans", "fee_structure", "fee_structures_new",
        "fee_categories", "fee_collections", "fee_invoices", "fee_payments",
        "notices", "notifications", "notice_reads",
        "certificates", "report_cards", "generated_admit_cards",
        "staff_leaves", "student_leaves", "leave_applications",
        "student_queries", "student_documents", "student_scholarships",
        "school_settings", "school_calendar", "calendar_events",
        "weak_students", "scholarships",
        "setup_wizards", "subscriptions", "website_config", "website_settings",
        "syllabus_progress", "syllabus_updates", "syllabus",
        "complaints", "support_issues", "testimonials"
    ]

    export_data = {
        "exported_at": datetime.now(timezone.utc).isoformat(),
        "exported_by": current_user.get("email", current_user.get("id")),
        "app": "schooltino",
        "collections": {}
    }
    ...
    export_data["meta"] = {
        "total_collections_exported": len(export_data["collections"]),
    ...
    return JSONResponse(
        ...
        headers={"Content-Disposition": "attachment; filename=schooltino_full_export.json"}
    )

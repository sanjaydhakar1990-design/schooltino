...
export default function TeachTinoDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [classesRes, noticesRes, subjectsRes, notificationsRes, myClassesRes, timetableRes] = await Promise.allSettled([
        ...
        axios.get(`${API}/timetable/teacher/${user?.id}?school_id=${user?.school_id}`, { headers }).catch(() => ({ data: { exists: false } }))
      ]);
      ...
      if (timetableRes.status === 'fulfilled' && timetableRes.value.data?.exists !== false) {
        setTeacherTimetable(timetableRes.value.data);
      }
  ...
  const skipChapter = async (chapter) => {
    ...
    try {
      ...
      await axios.post(
        `${API}/syllabus-progress/update`,
        {
          ...
          status: 'skipped',
          topics_covered: [],
          notes: 'Skipped by teacher'
        },
        { headers: { Authorization: `Bearer ${token}` } }
      );
      toast.success(`⏭️ Chapter ${chapter.number} skipped`);
      loadSyllabusForSubject(selectedSubjectForSyllabus, syllabusRange);
    } catch (error) {
      toast.error('Skip karne mein error');
    }
  };
                  ...
                  <div key={idx} className="p-4 bg-white rounded-lg border border-purple-200 hover:shadow-md transition-shadow">
                    ...
                    <div className="mt-3 flex gap-2">
                      ...
                      <Button
                        ...
                        className="text-xs"
                        onClick={() => {
                          setSelectedClassForAttendance(availableClasses.find(c => c.name === subject.class_name || c.class_name === subject.class_name));
          ...
          <Card className="mb-6 bg-gradient-to-br from-blue-50 to-white border-blue-200">
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center gap-2 text-blue-900 text-base">
                ...
                Aaj ka Timetable ({new Date().toLocaleDateString('hi-IN', { weekday: 'long' })})
              </CardTitle>
            </CardHeader>
            <CardContent>
              {(() => {
                const today = new Date().toLocaleDateString('en-US', { weekday: 'long' });
                const todaySchedule = teacherTimetable.schedule?.[today] || [];
                if (todaySchedule.length === 0) {
                  return <p className="text-sm text-gray-400 text-center py-3">Aaj koi class nahi hai 😊</p>;
                }
                return (
                  <div className="space-y-2">
                    {todaySchedule.map((period, idx) => (
                      <div key={idx} className="flex items-center gap-3 p-2 bg-white rounded-lg border border-blue-100">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-gray-500">{period.class_name || period.class_id}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <details className="mt-3">
                ...
                <div className="mt-2 space-y-2">
                  {['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'].map(day => {
                    const daySchedule = teacherTimetable.schedule?.[day] || [];
                    if (daySchedule.length === 0) return null;
                    return (
                      <div key={day}>
                        <p className="text-xs font-bold text-gray-500 uppercase mt-2">{day}</p>
                        <div className="flex flex-wrap gap-1">
                          {daySchedule.map((p, i) => (
                            <span key={i} className="text-xs bg-blue-50 border border-blue-100 px-2 py-1 rounded">
                              P{p.period}: {p.subject} ({p.class_name || ''})
                            </span>
                          ))}
                        </div>
                      </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    setHomeworkForm({ ...homeworkForm, class_id: e.target.value });
                  }}
                  className="w-full mt-1 p-2 border rounded-lg"
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    setHomeworkForm({ ...homeworkForm, subject: e.target.value });
                    // Auto-fill class if this subject is assigned to only one class
                    if (e.target.value) {
                      const matchingSubs = mySubjects.filter(s => (s.subject_name || s.subject) === e.target.value);
                      if (matchingSubs.length === 1 && matchingSubs[0].class_id) {
                        setHomeworkForm(prev => ({ ...prev, subject: e.target.value, class_id: matchingSubs[0].class_id }));
                      }
                    }
                  }}
                  className="w-full mt-1 p-2 border rounded-lg"
                >
                  ...
                  {[...new Set(mySubjects.map(s => s.subject_name || s.subject))].filter(Boolean).map(subName => (
                            ...
                            <div key={chapter.number} className="p-4 rounded-lg border bg-white">
                              <div className="flex items-start justify-between gap-2">
                                ...
                                <Badge className={statusStyles} data-testid={`syllabus-status-${chapter.number}`}>
                                  {status === 'completed' ? '✅ Completed' : status === 'in_progress' ? '📖 In Progress' : status === 'skipped' ? '⏭️ Skipped' : 'Not Started'}
                                </Badge>
                              </div>
